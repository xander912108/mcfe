...
export const pathApi = {
  async getProgress(userId: string): Promise<PathProgress | undefined> { await simulateLatency(150); return pathProgressMap[userId]; },
  async getTimeline(userId: string): Promise<PathStep[]> {
    await simulateLatency(100);
    console.log(`[MOCK] Load timeline for ${userId}`);
    return pathSteps;
  },
  async getDailyStep(userId: string): Promise<DailyStep | undefined> { await simulateLatency(100); return dailySteps[userId]; },
